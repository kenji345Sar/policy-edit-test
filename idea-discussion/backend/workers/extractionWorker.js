...
- 具体的な状況と影響を記述し、問題の本質を捉えやすくする
  * 良い例：「子育て世帯の親は、保育施設の空き状況を確認するために複数の施設に個別に重要論点合わせる必要があり、就労と育児の両立に大きな時間的負担となっている。この問題は子育て支援サービスについての会話で、ユーザーが先週5つの保育園に個別に電話をかけて空き状況を確認するのに半日かかったという具体的な体験を述べた際に明らかになった。」
...
- 現状と理想の状態を明確に記述し、そのギャップを問題として定義する
  ...
  * 悪い例：「災害情報が不足している」（現状と理想のギャップが不明確で背景情報がない）
- 具体的な状況と影響を記述し、問題の本質を捉えやすくする
  * 良い例：「子育て世帯の親は、保育施設の空き状況を確認するために複数の施設に個別に重要論点合わせる必要があり、就労と育児の両立に大きな時間的負担となっている。この問題は地域の子育て支援フォーラムで、共働き世帯の親が保育園探しのために休暇を取らざるを得なかった具体的な事例や、電話での重要論点合わせが営業時間内に限られるため仕事中に対応できないといった実体験が共有された投稿から抽出された。」
  * 悪い例：「保育施設の情報収集が大変」（具体的な影響が不明確で背景情報がない）
- 解決策の先走りや抽象的な表現を避ける
- 感情的な表現や主観的な解釈を排し、客観的な事実に基づいて記述する
- 問題の範囲を明確にし、多様な視点からの議論を促す表現を心がける
...
- 実現可能性や費用対効果といった制約条件も考慮する
  ...
  * 悪い例：「最新技術を使った防災システムを構築する」（実現可能性や費用対効果が考慮されていない）
- 曖昧な表現や抽象的な概念を避ける
  * 良い例：「保育施設の空き状況をリアルタイムで確認できるウェブポータルを構築し、検索条件に合わせた施設リストを表示する機能を実装する。この機能は地域の子育て支援フォーラムで、複数の親が実際に保育園探しで経験した具体的な困難（営業時間内の電話確認の難しさ、複数施設への重要論点合わせの手間など）を詳細に記述した投稿から抽出された解決策である。」
  * 悪い例：「保育施設の情報を改善する」（何をどう改善するか不明確で背景情報がない）
- 課題に対する具体的な応答として解決策を提示する
- 効果、リスク、実装に必要なステップを明確にする
